...
const mockAuthService = {
  ...
  sendForgotPasswordOtp: jest.fn().mockResolvedValue({ success: true }),
  verifyForgotPasswordOtp: jest.fn().mockResolvedValue({ success: true }),
  ...
  generate2FASecret: jest.fn().mockResolvedValue({ success: true, qr: 'base64' }),
...
describe('AuthController', () => {
  ...
  it('should verify forgot password otp', async () => {
    const result = await controller.verifyForgotPasswordOtp({
      email: 'john@example.com',
      otp: '123456',
    });
    expect(result.success).toBe(true);
  ...
  it('should verify email', async () => {
    const result = await controller.verifyEmail({ token: '1234' } as any);
    expect(result.success).toBe(true);
  });
