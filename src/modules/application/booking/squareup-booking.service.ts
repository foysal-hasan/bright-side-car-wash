...
export class SquareUpBookingService {
    ...
    private readonly getAllLocationCacheTimeTtlSeconds = 600; // 10 minutes (60 * 10)
    private readonly getServicesCacheTimeTtlSeconds = 600; // 10 minutes (60 * 10)
    ...
    async getLocations() {
        const cacheKey = RedisKeys.getAllLocationKey();
        try {
            // 1. Check if cached data exists
            const cachedLocations = await this.redis.get(cacheKey);
            if (cachedLocations) {
                return JSON.parse(cachedLocations);
            }

            // 2. Fetch from Square API on cache miss
            const response = await this.squareClient.locations.list();
            ...
            // 3. Cache the JSON string in Redis with a 10-minute EX (expiration in seconds)
            await this.redis.set(
                cacheKey,
                JSON.stringify(locations),
                'EX',
                this.getAllLocationCacheTimeTtlSeconds,
            );
    ...
    async getServices(query: GetServicesQueryDto) {
        const cacheKey = RedisKeys.getServicesKey({
        ...
        try {
            // 1. Check if cached data exists
            const cachedServices = await this.redis.get(cacheKey);
            if (cachedServices) {
            ...
            const result = this.toJsonSafe({ data: services, nextCursor: response.cursor || null });

            // 3. Cache the JSON string in Redis with EX (expiration in seconds)
            await this.redis.set(
                cacheKey,
                JSON.stringify(result),
                'EX',
                this.getServicesCacheTimeTtlSeconds,
            );
