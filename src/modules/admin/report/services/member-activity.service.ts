    ...
    try {
      ...
      users.forEach(user => {
        ...
        if (assignedCount > 0) {
          ...
          const convertedCount = user.assigned_leads.filter(lead => 
            lead.stage?.name?.toLowerCase().includes('converted')
          ).length;

          if (convertedCount > maxConvertedCount) {
            maxConvertedCount = convertedCount;
            topPerformerName = `${user.first_name} ${user.last_name}` || 'Unknown Staff';
          }
        }
      });

      return {
        activeStaffMembers: activeStaffCount,
        avgLeadsPerStaff: activeStaffCount > 0 ? Math.round(totalAssignedLeads / activeStaffCount) : 0,
        topPerformer: {
          name: topPerformerName,
          convertedCount: maxConvertedCount > -1 ? maxConvertedCount : 0,
        },
      };
    } catch (error) {
      throw new BadRequestException('Failed to calculate member highlights.');
    }
  }
